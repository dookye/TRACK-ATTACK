...
        } else {
            ...
            // Dies ist ein Fallback, falls der Web API Play-Befehl den Player
            // zwar mit dem Song befüllt, aber die Audio-Wiedergabe stumm lässt (iOS-Spezifika).
            if (spotifyPlayer) {
                spotifyPlayer.resume().then(() => {
                    console.log("[iOS Fallback] Zusätzliches resume() nach Web API Play ausgeführt.");
                }).catch(err => {
                    console.warn("[iOS Fallback] Zusätzliches resume() fehlgeschlagen:", err);
                });
            }
        }
    }).catch(error => {
